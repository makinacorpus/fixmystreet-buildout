...
eg: cd makina.fixmystreet && python2.4 bootstrap.py && bin/buildout -vvvvvvc <CONFIG_FILE>
...
Before doing anything in your project just after being installed, just source the environment file in your current shell::
    ...
    source $MT/zope/makina.fixmystreet/sys/share/minitage/minitage.env # env file is generated with $MT/bin/paster create -t minitage.instances.env makina.fixmystreet
...
::
    ...
    svn co ssh://gitorious-git@gitorious.makina-corpus.net/makinacorpusfixmystreet-buildout.git/minilays/makina.fixmystreet $MT/minilays/makina.fixmystreet
...
PROJECT SETTINGS
~~~~~~~~~~~~~~~~~~~~~~~~
- Think you have the most important sections of this buildout configuration in etc/makina.fixmystreet.cfg
Set the project developement  specific settings there
::

    etc/project/
    |-- makina.fixmystreet.cfg       -> your project needs (packages, sources, products)
    ...
    |-- makina.fixmystreet-kgs.cfg   -> Generated KGS for your project (minitage's printer or buildout.dumppickledversion)
...
::
    etc/apache/
    |-- 100-makina.fixmystreet.reverseproxy.conf                     -> a vhost for ruse with a standalone plone (even with haproxy in front of.)
    |-- 100-makina.fixmystreet.reverseproxy.deliverance.conf         -> a vhost for use with a plone behind a deliverance server.
    `-- apache.cfg
    etc/templates/apache/
    |-- 100-makina.fixmystreet.reverseproxy.conf.in                  -> Template for a vhost for ruse with a standalone plone (even with haproxy in front of.)
    `-- 100-makina.fixmystreet.reverseproxy.deliverance.conf.in      -> Template for a vhost for use with a plone behind a deliverance server.
...
KGS FILE
----------
We provide a part to generate the etc/makina.fixmystreet-kgs.cfg file.
...
To unlock the versions, cmment out the according statement ``etc/project/makina.fixmystreet-kgs}.cfg`` in the extends option of the makina.fixmystreet.cfg gile.
...
For example, to add a postgresql instance to your project, you will have to issue those steps:
    * $EDITOR minitage/minilays/makina.fixmystreet_minilay/makina.fixmystreet -> add postgresql-8.4 to the dependencies list
    * minimerge -v  makina.fixmystreet install what was not, and surely at least postgresql-8.4
    * minitage/bin/paster create -t minitage.instance.postgresql makina.fixmystreet
    * Then to start the postgres : zope/makina.fixmystreet/sys/etc/init.d/makina.fixmystreet_postgresql restart
